...
- **部署以「整目录」为单位**：模组目录除 `voicechat.dll`/`manifest.json` 外，还必须包含 `lang/`（i18n）、`panels/`（设置与管理员面板定义）与 `icons/`（HUD 状态图标 PNG）。只拷 dll+manifest 会部署出「空壳模组」——表现为面板打不开、文案回落显示原始 i18n 键、HUD 只有文字没有图标（2026-09-26 真机即此问题）。`scripts/Invoke-VoiceChatSmokeTest.ps1` 的 `Copy-ModArtifact` 已改为整目录树拷贝并把三个目录列为必备项（缺一即报错），同时保留安装目录下的 `config/`。
