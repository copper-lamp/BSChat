...
客户端内置进服自检 `vc::client::SmokeTest`（`src/client/entry/SmokeTest.{h,cpp}`）：`ClientJoinLevelEvent` 时武装，握手完成后自动走一次真实上行（`setTalking(true)` + 定时 `submitPcm` 合成正弦）并等待服务端回传，把 `handshake`/`uplink`/`downlink`/`overall` 四行 `[smoke]` 结果写入客户端日志；`ClientExitLevelEvent` 时销毁。该类零 LeviLamina 依赖，日志经 `setLogSink` 注入的 `std::function` 输出，由 `ClientMod` 在进服时接到 `NativeMod::current()->getLogger()`。详见 [单设备自动冒烟测试](smoke-test.md)。
