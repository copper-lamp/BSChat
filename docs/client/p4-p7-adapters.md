...
当前 xmake 可见的 LeviLamina 包只声明 `target_type=client`，并拉取 `bedrockdata ...-client.17`；仓库 checkout 中没有 `libs/third_party/SDK`，也不依赖 ImGui / Dear-OreUI 头文件。客户端事件与渲染 API 已确认为可用（`ll/api/event/client|input|world|render`，渲染事件通道经真机验证）。现已加入 `src/client/audio/WasapiAudio.*`：它通过 Windows `MMDeviceEnumerator` 真实探测默认捕获/播放端点，向上层返回 HRESULT 和明确状态。该边界不伪造 PCM stream、游戏线程或 UI 回调。
